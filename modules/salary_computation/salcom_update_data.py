...
def salcom_upd_scr():
    ...
    def update():
        ...
        xc=g1.get()
        xd=j1.get()
        sql="update salary_computation set dept_id=%d,month='%s',net_payable='%s' where emp_id=%d"%(xb,xc,xd,xa)
        cur.execute(sql)
        db.commit()
    ...
    aa=Label(t,text='UPDATE DATA',font=('algerian',25),bg='mediumpurple1')
    aa.place(x=250,y=60) 
    ...
    n=Button(t,text='Update',command=update,bg='mediumpurple1')
    n.place(x=250,y=450)
