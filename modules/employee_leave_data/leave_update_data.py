...
def leave_upd_scr():
    t = ttkb.Toplevel()
    t.title('EMPLOYEE LEAVE - Update')
    xt = []

    def filldata():
        db = get_connection()
        cur = db.cursor()
        sql = "select emp_id from emp_leave_data order by emp_id"
        cur.execute(sql)
        data = cur.fetchall()
        for res in data:
            xt.append(res[0])
    ...
    def update():
        ...
        xc = (g1.get())
        xd = (j1.get())
        sql = "update emp_leave_data set month='%s',no_of_leaves='%s',type='%s' where emp_id=%d" % (xb, xc, xd, xa)
        cur.execute(sql)
        db.commit()
    ...
    ttkb.Label(main_frame, text="No. of Leaves", font=('Segoe UI', 11)).grid(row=4, column=0, sticky=W, pady=8, padx=(0, 15))
    g1 = ttkb.Entry(main_frame, width=30, bootstyle="warning")
    g1.grid(row=4, column=1, pady=8)

    ttkb.Label(main_frame, text="Type", font=('Segoe UI', 11)).grid(row=5, column=0, sticky=W, pady=8, padx=(0, 15))
